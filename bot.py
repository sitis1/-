...
async def pay_info(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if code:
        ...
        if has_used_promo(uid, code):
            ...
            return MENU
        # Код валиден и не использован — помечаем как использованный сразу
...
async def hf_generate(prompt: str) -> bytes | None:
    try:
        ...
        if resp.status_code == 200:
            ...
            logger.error(f"HF error {resp.status_code}: {resp.text[:200]}")
            return None
    except Exception as e:
...
async def image_generate(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if "аватарк" in text.lower():
        return await img_avatar_start(update, ctx)
    if "обложк" in text.lower():
...
async def img_banner_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Опиши свою специализацию и стиль:\n\n"
        "<i>Пример: веб-разработчик, синий цвет, современный стиль, ноутбук и код</i>",
        parse_mode="HTML"
    )
    return IMG_BANNER
...
async def img_banner_generate(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = f"professional portfolio banner for freelancer, {desc}, modern design, clean, gradient background, wide format, high quality"
    img = await hf_generate(prompt)
    if img:
        await msg.delete()
        await update.message.reply_photo(
            photo=img,
            caption="🖼 <b>Баннер для портфолио готов!</b>",
            parse_mode="HTML",
            reply_markup=main_menu_keyboard()
        )
    else:
...
async def img_avatar_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "<i>Пример: деловой стиль, светлый фон, уверенная улыбка, профессиональный вид</i>",
        parse_mode="HTML"
    )
    return IMG_AVATAR
...
async def img_avatar_generate(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = f"professional freelancer profile photo, headshot portrait, {desc}, clean background, high quality, photorealistic"
    img = await hf_generate(prompt)
    if img:
        await msg.delete()
        await update.message.reply_photo(
            photo=img,
            caption="👤 <b>Аватарка готова!</b>",
            parse_mode="HTML",
            reply_markup=main_menu_keyboard()
        )
    else:
...
async def img_cover_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Опиши тему и стиль:\n\n"
        "<i>Пример: дизайнер логотипов, яркие цвета, творческий стиль, Instagram</i>",
        parse_mode="HTML"
    )
    return IMG_COVER
...
async def img_cover_generate(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = f"social media cover banner for freelancer, {desc}, professional design, eye-catching, modern, wide format"
    img = await hf_generate(prompt)
    if img:
        await msg.delete()
        await update.message.reply_photo(
            photo=img,
            caption="📰 <b>Обложка для соцсетей готова!</b>",
            parse_mode="HTML",
            reply_markup=main_menu_keyboard()
        )
    else:
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            IMG_BANNER: [MessageHandler(filters.TEXT & ~filters.COMMAND, img_banner_generate)],
            IMG_AVATAR: [MessageHandler(filters.TEXT & ~filters.COMMAND, img_avatar_generate)],
            IMG_COVER:  [MessageHandler(filters.TEXT & ~filters.COMMAND, img_cover_generate)],
        },
